...
def test_coordinator_records_last_successful_poll() -> None:
    ...
    assert client.calls == [("0-203-0-11521", 10, UTC, 60, None)]
...
def test_coordinator_passes_weather_condition_interval() -> None:
    client = SuccessfulClient()
    entry = _config_entry()
    entry.data[CONF_SUPPORTED_ELEMENTS_BY_INTERVAL]["60"].append("ww")
    coordinator = ChmiDataUpdateCoordinator(
        hass=SimpleNamespace(),
        config_entry=entry,
        client=client,
